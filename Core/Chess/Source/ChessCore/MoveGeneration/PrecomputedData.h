...
namespace Chess::MoveGen
{
	struct ChessCoord
	{
		...
		constexpr ChessCoord( short rank, short file )
			: Rank( rank ), File( file )
		{
		}
		...
		constexpr ChessCoord& operator += ( const ChessCoord& other )
		{
			this->Rank += other.Rank;
			this->File += other.File;
			return *this;
		}
		constexpr friend ChessCoord operator + ( ChessCoord left, const ChessCoord& right )
		{
			left += right;
			return left;
		}
	...
	constexpr std::array<ChessCoord, 8> DirChanges = {
		ChessCoord( 1,  0 ),
		ChessCoord( 1,  1 ),
		ChessCoord( 0,  1 ),
		ChessCoord( -1,  1 ),
	...
	constexpr std::array<ChessCoord, 8> KnightCoordChanges = {
		ChessCoord( 2, -1 ),
		ChessCoord( 2,  1 ),
		ChessCoord( 1,  2 ),
		ChessCoord( -1,  2 ),
	...
	// King moves
	constexpr std::array<Bitboard, 64> CreateKingBitboards()
	{
		std::array<Bitboard, 64> result{ };
	...
	constexpr auto KingMoves = CreateKingBitboards();

	// Knight moves
	constexpr std::array<Bitboard, 64> CreateKnightBitboards()
	{
		std::array<Bitboard, 64> result{ };
	...
	constexpr auto KnightMoves = CreateKnightBitboards();

	// Pawn moves
	constexpr std::array<Bitboard, 64> CreatePawnBitboards( bool White )
	{
		std::array<Bitboard, 64> result{ };

		ChessCoord Change = White ? ChessCoord( 1, 0 ) : ChessCoord( -1, 0 );
		short StartRank = White ? 1 : 6;
